  ...
  const applyFilters = () => {
    ...
    if (filterType && filterType !== "all") {
      filtered = filtered.filter(
        (a) => (a.consultationType || "").toLowerCase() === filterType,
      );
    }
    if (filterDate) {
  ...
  const createMeeting = async (appt) => {
    try {
      ...
      setAppointments((prev) =>
        prev.map((a) => (a._id === appt._id ? updated : a)),
      );
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Manage and assign video consultations
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl overflow-hidden border border-gray-100 dark:border-gray-700 mb-8">
          ...
          <div className="p-4 bg-gray-50 dark:bg-gray-900/50">
            <div
              ...
              className="rounded-2xl overflow-hidden shadow-inner bg-black min-h-[600px]"
            >
          ...
          {loading ? (
            Array(6)
              ...
              .map((_, i) => (
                <div
                  key={i}
                  className="bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 animate-pulse"
                >
            ...
            <div className="col-span-full py-20 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-2 max-w-sm">
                There are currently no appointments matching the selected
                filters.
              </p>
            </div>
          ) : (
            appointments.map((appt) => (
              <div
                key={appt._id}
                className="bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all border border-gray-100 dark:border-gray-700 group"
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  >
                    {(appt.consultationType || "").toLowerCase() === "video" ? (
                      <Video size={24} />
                    ) : (appt.consultationType || "").toLowerCase() ===
                      "phone" ? (
                      <Phone size={24} />
                    ) : (
                  ...
                  {(appt.consultationType || "").toLowerCase() === "video" && (
                    <div className="pt-3 border-t border-gray-100 dark:border-gray-700">
                      <a
                        href={
                          appt.meetingUrl
                            ? `${appt.meetingUrl}/static/dialInInfo.html?room=${appt.meetingUrl.split("/").pop()}`
                            : `https://meet.jit.si/static/dialInInfo.html?room=CareLine360-${appt._id}`
                        }
                        target="_blank"
                ...
                {(appt.consultationType || "").toLowerCase() === "video" ? (
                  ...
                  appt.status === "confirmed" ? (
                    ...
                    <a
                      ...
                      className="w-full block text-center py-4 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 transition-all"
                    >
