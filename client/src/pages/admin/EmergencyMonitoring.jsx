...
const StatusBadge = ({ status }) => {
  const styles = {
    ...
    ARRIVED:
      "bg-violet-50 text-violet-700 border-violet-200 dark:bg-violet-900/20 dark:text-violet-400 dark:border-violet-900/30",
    RESOLVED:
...
const EmergencyMonitoring = () => {
  ...
  // Filtered + paginated emergencies
    ...
    if (searchTerm.trim()) {
      ...
      list = list.filter(
        (e) =>
          ...
          (e.patient?.phone || "").includes(q),
      );
    }
    return list;
  }, [emergencies, statusFilter, searchTerm]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-left">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium">
            Real-time situational awareness and responder orchestration
          </p>
        </div>
      ...
      <div className="flex items-center gap-4">
        <div
          className={`px-4 py-2 rounded-lg cursor-pointer ${activeTab === "monitor" ? "bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white font-bold" : "text-slate-500 dark:text-slate-400"}`}
          onClick={() => setActiveTab("monitor")}
        >
          Monitor
        </div>
        <div
          className={`px-4 py-2 rounded-lg cursor-pointer ${activeTab === "config" ? "bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white font-bold" : "text-slate-500 dark:text-slate-400"}`}
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm">
            <div className="flex flex-col md:flex-row gap-4">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search by patient name, description or phone..."
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {paginated.length === 0 ? (
                    <tr>
                      ...
                      >
                        No emergency cases match your criteria.
                      </td>
                    </tr>
                  ) : (
                    paginated.map((e) => (
                      ...
                      >
                        <td className="py-5 px-6">
                          <div className="font-bold text-slate-900 dark:text-white text-base">
                            {e.patient?.fullName || "Identity Pending"}
                          </div>
                          <div className="text-xs text-slate-500 dark:text-slate-500 font-medium max-w-[280px] truncate italic">
                            "{e.description}"
                          </div>
                        </td>
                        <td className="py-5 px-6">
                        ...
                        <td className="py-5 px-6">
                          <div className="flex items-center gap-2 text-xs font-bold text-slate-500 group">
                            ...
                            <span className="font-mono tracking-tighter">
                              {e.latitude?.toFixed(5) || 0},{" "}
                              {e.longitude?.toFixed(5) || 0}
                            </span>
                          </div>
                        </td>
                        <td
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden text-left p-6">
          ...
          >
            ...
            <input
              ...
              onChange={(e) =>
                handleNewHospitalChange("contact", e.target.value)
              }
            ...
            <input
              ...
              onChange={(e) =>
                handleNewHospitalChange("address", e.target.value)
              }
              placeholder="Address"
          ...
          <div>
            <h4 className="text-sm font-bold mb-2">
              Saved Hospitals (MongoDB)
            </h4>
            {hospitals.length === 0 ? (
              <p className="text-slate-500 italic">
                No hospitals saved in database.
              </p>
            ) : (
              <ul className="space-y-3">
                {hospitals.map((h) => (
                  <li
                    key={h._id}
                    className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-2xl flex justify-between items-center transition-all hover:shadow-md"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <div className="flex items-center gap-1 text-[10px] font-medium text-slate-500">
                            ...
                            {h.address || "No Address Listed"}
                          </div>
                          <div className="flex items-center gap-1 text-[10px] font-medium text-slate-500">
                            <Phone size={10} className="text-teal-500" />{" "}
                            {h.contact || "No Contact Listed"}
                          </div>
                        </div>
                        <div className="text-[9px] font-mono text-slate-400 mt-1 uppercase tracking-tighter">
                          Node: {h.lat.toFixed ? h.lat.toFixed(5) : h.lat},{" "}
                          {h.lng.toFixed ? h.lng.toFixed(5) : h.lng}
                        </div>
                      </div>
                    </div>
                    <button
      ...
      {isModalOpen && selectedEmergency && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div
            className="absolute inset-0 bg-slate-950/70 animate-in fade-in duration-300"
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-800 z-10">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                    Registry Ref: {selectedEmergency._id}
                  </p>
                </div>
              </div>
              <button
                ...
                <div className="lg:col-span-5 space-y-6 text-left">
                  <div className="bg-slate-50 dark:bg-slate-950 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-inner space-y-5">
                    <div className="flex items-start gap-4">
                      <div className="w-10 h-10 flex items-center justify-center rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-teal-600 shadow-sm">
                        <User size={20} />
                      </div>
                      <div>
                        ...
                        <p className="font-bold text-lg text-slate-900 dark:text-white">
                          {selectedEmergency.patient?.fullName ||
                            "Identity Pending"}
                        </p>
                    ...
                    {selectedEmergency.responseTime && (
                      <div className="bg-emerald-500/10 dark:bg-emerald-400/10 p-5 rounded-xl border border-emerald-500/20 flex flex-col items-center gap-0.5 group overflow-hidden relative transition-all">
                        <p className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-[0.2em] relative">
                          Network Response Delta
                        </p>
                        <p className="text-3xl font-black text-emerald-700 dark:text-emerald-300 relative tracking-tighter">
                          {selectedEmergency.responseTime}
            ...
            <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 flex justify-end gap-3 shrink-0">
              ...
              {selectedEmergency.status !== "RESOLVED" && (
                <button
                  onClick={() =>
                    updateStatus(selectedEmergency._id, "RESOLVED")
                  }
