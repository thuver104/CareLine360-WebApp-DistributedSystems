...
const ManageUsers = () => {
  ...
  useEffect(() => {
    if (selectedUser && selectedUser.role === "doctor") {
      api
        .get(`/ratings/doctor/${selectedUser._id}`)
  ...
  const fetchUsers = async () => {
    ...
    try {
      const response = await api.get(
        `/admin/users?page=${page}&limit=10&search=${searchTerm}&role=${roleFilter}`,
      );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">
            Total {totalUsers} users registered in the system
          </p>
        </div>
        <button
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-200 dark:border-slate-800">
                <th className="py-4 px-6 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">
                  User Identity
                </th>
                <th className="py-4 px-6 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">
                  Designation
                ...
                <th className="py-4 px-6 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest text-right">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                Array(5)
                  ...
                  .map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      ...
                      <td className="py-5 px-6">
                        <div className="h-8 bg-slate-100 dark:bg-slate-800 rounded w-24 ml-auto"></div>
                      </td>
                    </tr>
                  ))
              ) : users.length === 0 ? (
                <tr>
                  <td
                    colSpan="4"
                    className="py-20 text-center text-slate-500 dark:text-slate-400 font-medium italic"
                  >
                ...
                users.map((user) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6">
                      <span
                        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${user.status === "ACTIVE" ? "bg-teal-50 text-teal-700 dark:bg-teal-900/20 dark:text-teal-400" : "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400"}`}
                      >
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${user.status === "ACTIVE" ? "bg-teal-500 shadow-[0_0_8px_rgba(20,184,166,0.6)]" : "bg-slate-400"}`}
                        ></div>
                        {user.status}
                      </span>
                    </td>
                    ...
                    >
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="p-2 rounded-lg hover:bg-teal-50 dark:hover:bg-teal-900/20 text-slate-400 hover:text-teal-600 dark:hover:text-teal-400 transition-all"
                          title="Edit User"
                        >
                          <Pencil size={18} />
                        </button>
                        <button
                          ...
                          className="p-2 rounded-lg hover:bg-amber-50 dark:hover:bg-amber-900/20 text-slate-400 hover:text-amber-500 dark:hover:text-amber-400 transition-all"
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/10">
            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
              ...
              <span className="text-slate-900 dark:text-white font-bold">
                {(page - 1) * 10 + 1} - {Math.min(page * 10, totalUsers)}
              </span>{" "}
              of{" "}
              <span className="text-slate-900 dark:text-white font-bold">
                {totalUsers}
            ...
            <div className="flex-shrink-0 px-8 pt-8 pb-6 border-b border-slate-100 dark:border-slate-800 flex items-center gap-5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  <span
                    className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-widest border ${
                      selectedUser.role === "admin"
                        ? "bg-rose-50 text-rose-600 border-rose-100 dark:bg-rose-900/40 dark:text-rose-400 dark:border-rose-900/30"
                        : selectedUser.role === "doctor"
                          ? "bg-blue-50 text-blue-600 border-blue-100 dark:bg-blue-900/40 dark:text-blue-400 dark:border-blue-900/30"
                          : selectedUser.role === "responder"
                            ? "bg-indigo-50 text-indigo-600 border-indigo-100 dark:bg-indigo-900/40 dark:text-indigo-400 dark:border-indigo-900/30"
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-widest ${selectedUser.status === "ACTIVE" ? "bg-teal-50 text-teal-600 dark:bg-teal-900/20 dark:text-teal-400" : "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400"}`}
                  >
                    {selectedUser.status}
                  </span>
                </div>
              ...
              <button
                onClick={() => setShowDetailModal(false)}
                className="self-start p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors flex-shrink-0"
              >
                <X size={20} />
              </button>
              <button
                onClick={() => openEditModal(selectedUser)}
                className="self-start px-3 py-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold flex items-center gap-1.5 transition-all shadow-sm active:scale-95 flex-shrink-0"
              >
                <Pencil size={14} />
                Edit
              </button>
              <button
                ...
                className="self-start px-3 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold flex items-center gap-1.5 transition-all shadow-sm active:scale-95 flex-shrink-0"
              >
                <KeyRound size={14} />
                Reset Password
              </button>
                  ...
                  <section>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/40 p-5 rounded-2xl border border-slate-100 dark:border-slate-800/60 space-y-4">
                      <div className="flex items-center gap-3.5">
                        <div className="w-9 h-9 flex items-center justify-center rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm">
                          <Mail size={16} className="text-slate-500" />
                        </div>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
                          {selectedUser.email || "Email not listed"}
                        </span>
                      </div>
                      <div className="flex items-center gap-3.5">
                        <div className="w-9 h-9 flex items-center justify-center rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm">
                        ...
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
                          {selectedUser.phone || "Phone not listed"}
                        </span>
                      </div>
                      ...
                      <div className="bg-white dark:bg-slate-950 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4">
                        <p className="text-sm flex justify-between">
                          ...
                          <span className="font-bold text-slate-900 dark:text-white uppercase">
                            {selectedUser.profile.patientId}
                          </span>
                        </p>
                        <p className="text-sm flex justify-between">
                          <strong className="text-slate-500 font-bold">
                          ...
                          <span className="font-bold text-rose-600 dark:text-rose-400 px-2 py-0.5 bg-rose-50 dark:bg-rose-900/20 rounded-md">
                            {selectedUser.profile.bloodGroup || "UNK"}
                          </span>
                        </p>
                      ...
                      <div className="bg-white dark:bg-slate-950 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4">
                        ...
                        <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {selectedUser.profile.qualifications?.map(
                              (q, i) => (
                                <span
                                  key={i}
                                  className="px-3 py-1 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold"
                                >
                                  {q}
                                </span>
                              ),
                  ...
                  {selectedUser.role === "doctor" && selectedUser.profile && (
                    <>
                        ...
                        <div className="bg-slate-50 dark:bg-slate-800/40 p-5 rounded-2xl border border-slate-100 dark:border-slate-800/60">
                          <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed italic font-medium">
                            ...
                            {selectedUser.profile.bio ||
                              "Professional brief not provided by the doctor."}
                            "
                          </p>
                          <div className="mt-5 pt-5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-bold text-slate-900 dark:text-white italic">
                                {selectedUser.profile.experience} Years of
                                Service
                              </span>
                            </div>
                      ...
                      <section>
                        ...
                        <div className="space-y-2">
                          {selectedUser.profile.availabilitySlots?.length >
                          0 ? (
                            selectedUser.profile.availabilitySlots.map(
                              (slot, i) => (
                                <div
                                  key={i}
                                  className="flex justify-between items-center px-4 py-3 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm"
                                >
                                  <span className="text-[10px] font-bold uppercase text-slate-500">
                                    {slot.day}
                                  </span>
                                  <span className="text-xs font-bold text-teal-600 dark:text-teal-400 tracking-tight">
                            ...
                            <p className="text-xs text-slate-400 italic p-4 text-center border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-2xl">
                              No indexed availability slots
                            </p>
                          )}
                        </div>
                      </section>
                        ...
                        <div className="space-y-4">
                          {doctorReviews.length === 0 ? (
                            ...
                            doctorReviews.map((r, i) => (
                              <div
                                key={i}
                                className="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-800/60"
                              >
                      ...
                      <div className="space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-4 bg-rose-50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-900/30 rounded-2xl">
                            <p className="text-[10px] font-bold text-rose-500 uppercase mb-3 tracking-widest">
                              Allergies
                            </p>
                            <div className="flex flex-wrap gap-2">
                              {selectedUser.profile.allergies?.length > 0 ? (
                                selectedUser.profile.allergies.map((a, i) => (
                                  <span
                                    key={i}
                                    className="px-2 py-0.5 bg-rose-500/10 text-rose-600 dark:text-rose-400 rounded-lg text-[10px] font-bold"
                                  >
                                    {a}
                                  </span>
                                ))
                          ...
                          <div className="p-4 bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30 rounded-2xl">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {selectedUser.profile.chronicConditions?.length >
                              0 ? (
                                selectedUser.profile.chronicConditions.map(
                                  (c, i) => (
                                    <span
                                      key={i}
                                      className="px-2 py-0.5 bg-amber-500/10 text-amber-600 dark:text-amber-400 rounded-lg text-[10px] font-bold"
                                    >
                                      {c}
                                    </span>
                                  ),
          ...
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-md p-8 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleEditUser} className="space-y-4">
              ...
              <button
                ...
                className="w-full bg-teal-600 hover:bg-teal-700 disabled:opacity-60 text-white py-3.5 rounded-xl font-bold mt-4 shadow-lg shadow-teal-500/20 transition-all active:scale-95 flex items-center justify-center gap-2"
              >
                <Save size={18} />
            ...
            {!resetResult ? (
              <>
                    ...
                    <div className="flex gap-2">
                      <div className="relative flex-1">
                        <input
                          ...
                          className="w-full px-4 py-2.5 pr-10 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all text-slate-900 dark:text-white font-mono"
                      ...
                      <button
                        ...
                        className="px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 transition-all flex items-center gap-1.5 text-xs font-bold"
                ...
                <button
                  ...
                  className="w-full bg-amber-500 hover:bg-amber-600 disabled:opacity-60 text-white py-3.5 rounded-xl font-bold mt-6 shadow-lg shadow-amber-500/20 transition-all active:scale-95 flex items-center justify-center gap-2"
                >
                  <KeyRound size={18} />
              ...
              <div className="space-y-4">
                {/* Show new password */}
                <div className="p-5 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-900/30 rounded-xl text-center">
                  <p className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest mb-2">
                    New Password
                  </p>
                  <div className="flex items-center justify-center gap-2">
                    <p className="text-xl font-black text-emerald-700 dark:text-emerald-300 font-mono tracking-wider">
                      {resetResult.newPassword}
                    </p>
                    <button
                      onClick={() => copyToClipboard(resetResult.newPassword)}
                      className="p-1.5 rounded-lg hover:bg-emerald-100 dark:hover:bg-emerald-800/30 text-emerald-600 transition-colors"
                      title="Copy password"
                    >
                      <Copy size={16} />
                    </button>
                ...
                >
                  {resetResult.notificationMethod === "email" && (
                    <div className="flex items-center gap-2">
                      <Mail size={16} />
                      <span>
                        Password sent via email to{" "}
                        <strong>{resetResult.userEmail}</strong>
                      </span>
                    </div>
                  )}
                  {resetResult.notificationMethod === "email_failed" && (
                    <div className="flex items-center gap-2">
                      <Mail size={16} />
                      <span>
                        Email delivery failed. Please share the password
                        manually.
                      </span>
                    </div>
                  )}
                  {resetResult.notificationMethod === "manual_phone" && (
                    <div className="flex items-center gap-2">
                      <Phone size={16} />
                      <span>
                        No email found. Please share password via phone:{" "}
                        <strong>{resetResult.userPhone}</strong>
                      </span>
                    </div>
                  )}
                  {resetResult.notificationMethod === "none" && (
                    <span>
                      No contact info found. Share the password manually.
                    </span>
                  )}
                </div>
                ...
                <button
                  onClick={() => setShowResetModal(false)}
                  className="w-full bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 py-3 rounded-xl font-bold transition-all active:scale-95"
                >
                  Done
                </button>
              </div>
