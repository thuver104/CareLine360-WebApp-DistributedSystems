...
const StatCard = ({ title, value, icon, color, delay }) => (
  <div
    className="group relative bg-[var(--bg-surface)] border border-[var(--border)] p-6 rounded-3xl flex items-center gap-4 shadow-sm hover:shadow-xl hover:translate-y-[-4px] transition-all duration-300 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both"
    style={{ animationDelay: `${delay}ms` }}
  >
    <div
      className={`p-4 rounded-2xl ${color} bg-opacity-10 dark:bg-opacity-20 group-hover:scale-110 transition-transform duration-300`}
    >
      {icon}
    </div>
    <div>
      ...
      <h3 className="text-2xl font-black text-[var(--text-primary)] mt-0.5 tracking-tight">
        {value}
      </h3>
    </div>
    <div
      className={`absolute bottom-0 left-6 right-6 h-0.5 bg-gradient-to-r from-transparent via-current to-transparent opacity-0 group-hover:opacity-30 transition-opacity ${color.replace("bg-", "text-")}`}
    ></div>
  </div>
);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-[var(--text-secondary)] font-medium mt-1">
            System telemetry and active responder network status
          </p>
        </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--text-muted)]">
                    Historical case closure velocity
                  </p>
                </div>
              </div>
            ...
            <div className="group flex items-center justify-between p-6 bg-[var(--bg-subtle)] rounded-2xl border border-[var(--border)] transition-all hover:border-blue-500/30">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--text-muted)]">
                    Active doctors vs total patients
                  </p>
                </div>
              </div>
              <div className="text-right">
