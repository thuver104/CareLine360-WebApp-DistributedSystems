...
const NAV_MAIN = [
  ...
  { id: "Appointments", icon: CalendarCheck,   label: "Appointments", hasBadge: true },
  { id: "My Patients",  icon: Users,           label: "My Patients"  },
  { id: "Availability", icon: CalendarClock,   label: "Availability" },
  { id: "Analytics",    icon: BarChart3,       label: "Analytics"    },
];

// Quick action items — navigate to Appointments and open the correct modal
const NAV_ACTIONS = [
  { id: "Medical Records",       icon: FileText,       label: "Medical Records"       },
  { id: "Generate Prescription", icon: Pill,           label: "Generate Prescription" },
  { id: "Chat",                  icon: MessageSquare,  label: "Chat with Patient"     },
];

export default function Sidebar({ section, setSection, doctor, pendingCount = 0, onQuickAction }) {
  ...
  return (
    <aside className="cl-sidebar" style={{ width: collapsed ? "70px" : "256px" }}>
      ...
      <button
        ...
        onClick={() => { setSection("Dashboard"); navigate("/doctor/dashboard"); }}
        title="Go to Dashboard"
      >
        <div className="grid h-10 w-10 shrink-0 place-content-center rounded-xl
        ...
        {NAV_MAIN.map(({ id, icon: Icon, label, hasBadge }) => {
          ...
          return (
            ...
            >
              ...
              {!collapsed && (
                <span className="text-sm font-medium truncate">{label || id}</span>
              )}
              {showBadge && !collapsed && (
      ...
      {collapsed ? (
        ...
        >
          {doctor?.avatarUrl
            ? <img src={doctor.avatarUrl} alt="" className="h-10 w-10 rounded-full object-cover" />
            : initials}
        </button>
      ) : (
        <div
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-500 dark:text-teal-200 truncate">{specialty}</p>
            </div>
